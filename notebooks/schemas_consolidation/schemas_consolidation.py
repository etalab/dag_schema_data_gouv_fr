...
def get_schema_dict(schema_name: str, schemas_catalogue_list: List[dict]) -> Optional[dict]:
...
def add_schema_default_config(schema_name: str, config_path: str, schemas_catalogue_list: List[dict]) -> None:
...
def save_validata_report(res, report, version, schema_name, dataset_id, resource_id, validata_reports_path):
    save_report = {}
...
def is_validata_valid_row(row, schema_url, version, schema_name, validata_reports_path) :
    if row['error_type'] is None : #if no error
        ...
        if report: save_validata_report(res, report, version, schema_name, row['dataset_id'], row['resource_id'], validata_reports_path)
        return res
    else :
        return False
...
def run_schemas_consolidation(
    ...
    search_url_base = '/'.join(api_url.split('/')[:-2]) + '/2/datasets/search/?q={search_word}'
    ...
    for schema_name in config_dict.keys() :
        ...
        if schema_config['consolidate'] == True :
            ...
            for search_word in search_words_list :
                df_search_word = parse_api_search(search_url_base.format(search_word=search_word), api_url)
            ...
            if len(df_list) > 0 :
                ...
                for version in schema_dict['versions'] :
                    ...
                    if version_name not in drop_versions :
                        ...
                        df['is_valid_v_{}'.format(version_name)] = df.apply(lambda row : is_validata_valid_row(row, schema_url, version_name, schema_name, validata_reports_path), axis=1)
