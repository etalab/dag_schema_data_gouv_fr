            ...
            for dataset_result in data["data"]:
                ...
                for res in dataset["resources"]:
                    ...
                    if ext not in ["csv", "xls", "xlsx"]:
                        ...
                        if (
                            not dataset["organization"]
                            and not dataset["owner"]
                        ):
...
def make_validata_report(
    rurl, schema_url, validata_base_url=VALIDATA_BASE_URL
):
    r = requests.get(
...
def is_validata_valid_row(
    row, schema_url, version, schema_name, validata_reports_path
):
...
def run_schemas_consolidation(
    ...
    search_url_base = (
        "/".join(api_url.split("/")[:-2])
        + "/2/datasets/search/?q={search_word}"
    )
    ...
    for schema in schemas_catalogue_list:
        print(
            "- {} ({} versions)".format(
                schema["name"], len(schema["versions"])
            )
        )
    ...
    for schema_name in config_dict.keys():
        ...
        if schema_config["consolidate"] == True:
            ...
            if "consolidated_dataset_id" in schema_config.keys():
                datasets_to_exclude += [
                    schema_config["consolidated_dataset_id"]
                ]
    ...
    for schema_name in config_dict.keys():
        ...
        if os.path.exists(ref_table_path):
            ...
            if len(df_ref[df_ref["is_valid_one_version"] == True]) > 0:
                ...
                schema_data_path = Path(data_path) / schema_name.replace(
                    "/", "_"
                )
                ...
                print(
                    "{} -- ⚠️ No valid resource for this schema".format(
                        datetime.now()
                    )
                )
    ...
    for schema_name in config_dict.keys():
        ...
        if os.path.exists(schema_data_path):
            ...
            print(
                "{} -- ❌ No data downloaded for this schema.".format(
                    datetime.today()
                )
            )
