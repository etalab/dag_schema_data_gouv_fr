...
def add_most_recent_valid_version(df_ref: pd.DataFrame) -> pd.DataFrame:
    ...
    version_cols_list = [
        col for col in df_ref.columns if col.startswith("is_valid_v_")
    ]
...
def create_schema_consolidation_dataset(
    ...
    schema_title = get_schema_dict(schema_name, schemas_catalogue_list)[
        "title"
    ]
...
def add_resource_schema(
    ...
    try:
        url = api_url + "datasets/{}/resources/{}/".format(
...
def post_comment_on_dataset(dataset_id, title, comment, api_url):
    ...
    _ = requests.post(
        api_url + "discussions/", json=post_object, headers=HEADER
    )
...
def run_consolidation_upload(
        ...
        if os.path.exists(schema_consolidated_data_path):
            ...
            if ("publication" in schema_config.keys()) and schema_config[
                ...
                if "consolidated_dataset_id" not in schema_config.keys():
                    ...
                    consolidated_dataset_id = schema_config[
                        "consolidated_dataset_id"
                    ]
                ...
                print(
                    "{} -- ❌ No publication for this schema.".format(
                        datetime.today()
                    )
                )
            ...
            print(
                "{} -- ❌ No consolidated file for this schema.".format(
                    datetime.today()
                )
            )
    ...
    for schema_name in config_dict.keys():
        ...
        if os.path.isfile(ref_table_path):
            ...
            df_ref["is_schema_to_drop"] = df_ref.apply(
                is_schema_to_drop, axis=1
            )

            df_ref.to_csv(ref_table_path, index=False)

            print(
                "{} - ✅ Infos added for schema {}".format(
            ...
            for idx, row in df_ref.iterrows():
                if row["is_schema_version_to_update"]:
                    ...
                    if resource_update_success == True:
                        title = (
                            "Mise à jour de la version de la métadonnée schéma"
                        )
                        comment = updated_schema_comment_template.format(
                            resource_title=row["resource_title"],
                            schema_name=schema_name,
                            initial_version_name=row["initial_version_name"],
                            most_recent_valid_version=row[
                                "most_recent_valid_version"
                            ],
                        )
        ...
        if os.path.isfile(ref_table_path):
            ...
            for idx, row in df_ref.iterrows():
                ...
                if (
                    ...
                    validata_report_path += (
                        row["most_recent_valid_version"] + ".json"
                    )
                # Else, check if declarative version
                else:
                    # If so, put validation report from it
                    if (
                        row["initial_version_name"]
                        == row["initial_version_name"]
                    ):
                        validata_report_path += (
                            row["initial_version_name"] + ".json"
                        )
    ...
    reports_df = reports_df[
        ["schema_name"]
        + [col for col in reports_df.columns if col != "schema_name"]
    ].rename(
    ...
    for schema_name in config_dict.keys():
        ...
        if os.path.isfile(ref_table_path):
            ...
            df_ref["is_schema_added"] = (
                df_ref["is_schema_to_add"]
                & df_ref["resource_schema_update_success"]
            )
            df_ref["is_schema_dropped"] = (
                df_ref["is_schema_to_drop"]
                & df_ref["resource_schema_update_success"]
            )
    ...
    for schema_name in config_dict.keys():
        ...
        if os.path.isfile(ref_table_path):
            ...
            cols_to_sum += [
                col for col in df_ref.columns if col.startswith("is_")
            ]
            df_report = (
